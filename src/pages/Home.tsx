...
export default function Home() {
  ...
  /**
   * Corrección para iOS: 
   * Previene que el scroll automático tape la card al abrir el teclado.
   */
  useEffect(() => {
    if (isIdFocused && window.innerWidth < 768) {
      const timer = setTimeout(() => {
        const cardElement = document.getElementById('tarjeta-previa');
        if (cardElement) {
